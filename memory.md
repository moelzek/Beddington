...
- **Next gate:** 28 Jun demo build is underway. The core live loop (cry → soothe → quiet-check → "crying no longer detected") is confirmed working and audible on the Pi: a real cry through the USB mic scored 0.891 and triggered soothe; a file-fed `analyze --realtime` run resolves after 2 quiet checks. Speaker output is via the Anker Bluetooth speaker as an interim (MAX98357 reopened for the demo, but the 3W speaker is not yet soldered to the amp output). BME688 air sensor and the HC-SR501 PIR are wired and verified; OLED face and one micro-servo are a bench-only demo exception; hot compute stays in a vented base, only cool parts in the plush. Next: room temp + motion into a spoken summary, then the local AI brain. Nursery camera deployment, active safety claims, face-covered claims, and night-vision remain gated; radar (MR60BHA2) stays post-demo.
