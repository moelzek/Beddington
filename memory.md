...
| Compute | Pi 5 (4GB, on hand) + Camera Module 3 + **AI HAT+ 26 TOPS (Hailo-8) — on hand & fitted to the Pi 5, used for v0 vision** (Hailo-accelerated object detection). Classical OpenCV state-machine + timing logic still run on the CPU. Cloud LLM optional, for prose only. *(Reverses the earlier "no AI HAT+ for v0 / CPU-first" call — Mo's decision 13 Jun: the HAT is now in hand.)* |

## Kit list (all ordered / in hand as of 13 Jun 2026)

**All kit ordered and physically in hand** — the Pi 5 rig is present at Workshop #1. Pi 5 4GB (**have**) · **AI HAT+ 26 TOPS / Hailo-8 (have — fitted to the Pi 5, used for v0 vision)** · Active cooler + 27W USB-C PSU (**have**) · Camera Module 3 ×2 (**have**) · 64GB A2 microSD (32GB **acquired**) · **Pi-5-compatible** camera cable (~£3) · top-down mount/clamp (~£15–30, single biggest reliability lever) · LED/ring light (~£15) · optional GPIO LED/buzzer (~£5). *(Pi AI Camera IMX500 (~£70) is an unused on-sensor alternative — not needed now the AI HAT+ is in hand.)*
...
- **2026-06-13** — **Compute decision REVERSED (Mo's call): AI HAT+ now used for v0.** Mo has the **Raspberry Pi AI HAT+ 26 TOPS (Hailo-8)** in hand and fitted to the Pi 5, so v0 vision now runs **Hailo-accelerated object detection** (the state machine + timing logic stay on the CPU). This supersedes the earlier 13 Jun "no AI HAT+ for v0 / CPU-first, defer 13 TOPS/Hailo-8L" call. Propagated to Locked decisions + Kit list above and to context.md, DEVLOG.md, agents.md, BUILD.md (incl. a new "fit the AI HAT+" step), lab-witness-v0-build-doc.md, lab-witness-hardware-inventory.md, and the three reviewer-skill source checklists (hardware/software/flomotion-cto) — **NB the packaged `.skill` archives still need repackaging to push this live.** Mirrored to engram memory. Spec corrected from the earlier 13 TOPS/Hailo-8L reference to **26 TOPS/Hailo-8**.
