...
- **Next gate:** 28 Jun demo build is underway. The core live loop (cry → soothe → quiet-check → "crying no longer detected") is confirmed working and audible on the Pi: a real cry through the USB mic scored 0.891 and triggered soothe; a file-fed `analyze --realtime` run resolves after 2 quiet checks. Speaker output is via the Anker Bluetooth speaker as an interim (MAX98357 reopened for the demo, but the 3W speaker is not yet soldered to the amp output). BME688 air sensor and the HC-SR501 PIR are wired and verified; OLED face and one micro-servo are a bench-only demo exception; hot compute stays in a vented base, only cool parts in the plush. Next: room temp + motion into a spoken summary, then the local AI brain. Nursery camera deployment, active safety claims, face-covered claims, and night-vision remain gated. The MR60BHA2 radar is now integrated as optional, off-by-default **non-medical** context only (presence, room brightness, distance, target count over WiFi); its respiratory/heart-rate vital signs are never routed into narration and stay bench-only (Tier 3, no vital-sign claims).
...
| AI orchestrator | **AI-orchestrator-first (Mo, 28 Jun)**: a local AI brain is on by default and runs the experience (soothe choice, sensor-fusion, narration, conversation). A deterministic cry→alert **reflex** always fires the alert instantly even if the AI lags. This replaces the old "works-with-AI-off" rule. Any cloud LLM is an optional quality upgrade behind a flag and receives derived text only — never raw audio/video. The reflex owns the alert; the AI runs the story. |
...
| VL53L0X laser distance/ToF sensor | 17, 20, 16, 15 | Expected alternate I²C pattern: 3.3V, GND, SCL/GPIO23, SDA/GPIO22 |
...
- **2026-06-28** — **Presence is radar-driven again; the PIR is being removed (Mo's call — the radar is more accurate).** Reverses the same-day PIR-only presence (commits e3dd360 / d3ea5fc, which had dropped the buried radar to kill phantom presence). New `radar_person_present` (`assistant.py`) trusts the radar's `person_present` flag **only when corroborated** by a real target (`target_distance_cm`, or `target_count` ≥ 1) **or** a plausible breathing lock (`radar_respiratory_rate` present), so bare-flag micro-vibration clutter still can't read as a phantom; the PIR's `motion_detected` no longer drives presence. New helpers `_radar_target_detected` / `_radar_breathing_lock`; the dashboard `_dashboard_fields` (`cli.py`) and the voice assistant both inherit it. Deployed to the Pi and **verified live**: with someone in the dim nursery the dashboard now shows `● someone present` (was stuck on `○ no one detected` while the radar was reading breathing/heart/distance). `test_radar_presence_requires_corroboration` rewritten; full suite green (asset-binary tests aside). Loosen to the bare flag later if it ever misses a still person. **Separately noted (not yet fixed):** after re-assembly the USB mic (nothing on USB; `paddington` crash-looping on PortAudio device -1) and the BME688 air sensor (nothing on I²C bus 1) are not detected even when switched on — physical reseat needed; the Hailo AI HAT remains present-but-unactivated (unused by design).
- **2026-06-28** — **Phone baby-monitor dashboard + supporting features all shipped (25 commits today; pre-assembly checkpoint).** A LAN-only phone dashboard (`lullaby live-view`, systemd service `lullaby-liveview`, stable URL `http://192.168.1.72:8088/?token=…`, token persisted): live MJPEG video, all-sensor readings overlay, per-sensor time-graph tabs (self-contained canvas, no CDN), persistent SQLite history (`sensor_store.py`, ~7-day, derived-only), a **Night digest** tab + `night-digest --speak` (`night_digest.py`, deterministic, no medical claims), **day/night smart mode** (`liveview.day_night_mode`, lux + hysteresis), **dual-camera day-eye/night-eye split** (cam 0 normal + cam 1 long-exposure, auto-switched on light via `_ModeBroker`), **motion→listen** fusion (`fusion.py` `FocusTracker` sharpens the non-cry sound diary on movement, cry alarm untouched — Codex-verified), and **Soothe controls** (white noise/heartbeat/music/uterine-whoosh buttons play looped through the speaker). Ask Paddington "how was the night?" → speaks the digest. All LAN-only, no recording, no audio off-device, no medical claims. **Sensor keep/remove for assembly:** KEEP (used) = BME688 (I²C air), HC-SR501 PIR (GPIO4 motion), 2× Camera Module 3 (CSI), USB mic, Anker BT speaker, and the MR60BHA2 radar kit (XIAO ESP32C6) — POWERED FROM THE PI via a USB-A → XIAO USB-C cable, with DATA over WiFi at 192.168.1.146 (not wired to the Pi GPIO; keep that USB power cable connected). It powers down/up with the Pi and re-joins WiFi + reconnects automatically a few seconds after boot. SAFE TO REMOVE (not read by any code) = VL53L0X, HC-SR04, INMP441, MAX98357 amp (playback goes to Anker BT, not the amp), OLED, servo/PCA9685; Hailo unused (optional, can stay). Sensor readers fail gracefully (return {}) so removing unused parts has zero software impact. On power-up the services auto-start and the radar reconnects over WiFi.
- **2026-06-28** — **Spoken medical disclaimer removed from the voice vitals answers (Mo's preference — personal bench device, not a medical product).** Paddington now answers e.g. *"From the radar, breathing about 16 breaths a minute, and heart rate about 90 beats per minute"* — no "not a medical or safety reading" warning. **Kept the safety property that matters:** answers still NEVER reassure or claim wellbeing (no fine/safe/healthy/normal/asleep — enforced by the `_no_reassurance` test) and never fabricate (honest "I don't have a clear reading" on no lock). **Written/logged surfaces keep their labels** (radar_vitals.py `RADAR_VITALS_DISCLAIMER`, events.json top-level disclaimer, `sensors-live` per-line marker) since those can be read out of context. Commit 80cdecb.
- **2026-06-28** — **Voice assistant now ANSWERS explicit vitals questions from the radar (Mo authorised — supersedes the earlier "assistant never answers vitals" rule).** Asking Paddington "what's her heart rate / breathing rate", or "how is the baby", returns a deterministic, clearly-labelled radar estimate: *"This is a rough radar estimate, not a medical or safety reading: breathing about 16 a minute, and heart rate about 90 beats per minute."* Guardrails kept intact (consistent with the bench_vitals authorisation): never paired with reassurance/wellbeing words (no fine/safe/healthy/normal/asleep); no interpretation of the numbers; honest *"I don't have a clear vitals reading"* when the radar has no lock (vitals keys only present when bench_vitals on + locked, still + close); unsupported vitals (oxygen, blood pressure, fever) decline honestly rather than returning air pressure or the breathing/heart estimate. The `_BABY_VITALS_PHRASES` route is narrow ("how is the baby", "is the baby okay/alright/doing", "check the baby") so "is the baby asleep/crying/hungry" fall through to the fallback, not vitals. `sensors-live` lines now also append "[raw bench data, not a medical or safety reading]" to any breathing/heart line. Reviewed adversarially (Codex CLI + 4-lens safety workflow); all blocker/major findings fixed. Deterministic, no LLM. 152 tests. Commit fc153fe.
- **2026-06-28** — **Paddington voice Q&A now WORKS reliably close-up (Mo confirmed correct spoken answers).** What finally cracked the unreliability: (1) **auto-calibration** — `listen-assistant` samples ~2s of room noise at startup and sets the speech threshold to ~1.5× the noise floor (min 0.015), ending the manual-threshold whack-a-mole (0.02→0.016→0.028→auto). (2) **Whisper priming** — `initial_prompt="Hey Paddington. What is the temperature, humidity, air pressure, brightness, or air quality?"` biases faster-whisper toward the wake word + topics, so marginal audio stopped mangling them ("Paddington" not "Vatican", "temperature" not "up virtual"). (3) `vad_filter=False` (it was trimming quiet speech). (4) whole-word keyword match ("shot" no longer false-matches "hot") + question normalization. (5) Fallback is just "Sorry, I didn't catch that. Please say it again." Verified live: "Hey Paddington, what's the temperature?" → "The room is about 27.8 degrees Celsius" (spoken). **Learnings:** lowering the threshold BELOW the noise floor backfires (utterances never close → 8s noise blobs bury speech); the room FAN was the dominant noise; SNR is the hard limit (reliable ~0.5m, far-field mic still needed for across-room); restart the service to recalibrate if the fan/room changes. Commits 7d2a095, 79fb9f4.
- **2026-06-28** — **Paddington always-on DEBUGGED + WORKING (Mo confirmed hearing replies).** The "not responding" was two bugs, not the audio: (1) the gas-enabled BME688 returns `{}` on ~half its reads, so a single-pass sensor snapshot left temperature blank and a temperature question wrongly answered presence ("active near the cot") — fixed by reading sensors over several passes in `_read_sensor_snapshot`; (2) removed the over-greedy "in the room" presence keyword in `answer_question`. The mic, transcription, and speak→Anker path were all fine (verified: speak works even with the mic InputStream open; `spoken: True` via ffplay). Added `listen-assistant --debug` (mic-energy heartbeats + transcripts + speak result) for diagnostics; turned back off on the service after. Confirmed live: "what's the temperature" → "about 27 degrees Celsius" (spoken + heard), "what's the humidity" → "47.2 percent". Idle mic noise floor ~0.018–0.023 vs energy threshold 0.02; real speech ~0.04–0.22.
- **2026-06-28** — **Paddington is always-on (Mo authorised).** Installed a systemd USER service on the Pi (`~/.config/systemd/user/paddington.service`, committed copy at `deploy/paddington.service`) that runs `listen-assistant` 24/7, auto-starts on boot (linger enabled for `lab`), and restarts on failure; logs to `~/paddington.log`. Verified: service is `active (running)`, loads Whisper, reaches "Listening". Wake word is **"Paddington"** (the companion lives in a Paddington bear). **Mic-ownership trade-off (Mo accepted):** one program owns the mic at a time, so this service and the cry-detection `listen`/`analyze` loop must not run on the same mic simultaneously — the proper fix is a future unified cry+wake loop on one mic stream. A speaker-loopback self-test didn't acoustically couple (mic didn't hear the distant Anker), but the full STT→wake→answer chain is already proven and the mic gets ~0.9 energy for nearby speech (threshold 0.02), so a person speaking near the mic triggers it. Manage with `systemctl --user status/restart/stop paddington`.
- **2026-06-28** — **Voice assistant COMPLETE — the "ears" landed.** "Hi Lullaby, what's the humidity?" now works fully spoken, fully on-device. New `lullaby listen-assistant` loops mic → energy VAD → faster-whisper tiny.en int8 → `ears.extract_wake_question` (fuzzy "lullaby" match; pure/testable, no audio imports) → `assistant.answer_question` (live sensors, deterministic) → Piper voice. Half-duplex (drops mic frames while speaking → never transcribes itself); captures at the mic's native rate and resamples to 16k for Whisper. webrtcvad was dropped (setuptools 81+ removed `pkg_resources`) in favour of energy VAD. Audio never leaves the device; the transcribed question always routes through the deterministic brain (no vitals branch → medical refusal holds, tested). RAM is fine — Ollama isn't needed (brain is deterministic), ~2GB free. Design + adversarial risk review came from a 3-agent workflow. Full STT→wake→answer chain self-tested on the Pi (Piper spoke the phrase → Whisper → "The humidity is about 49.4 percent."); the live mic loop starts clean. 124 tests pass. Install: `pip install faster-whisper` (added as the `ears` extra in pyproject).
- **2026-06-28** — **Voice assistant, layer 1 (brain + voice).** New `lullaby ask "<question>" [--speak]` reads a live sensor snapshot and answers **deterministically** (no LLM → no invented values): humidity, temperature, pressure, air-quality gas, brightness, presence (via the PIR↔radar scene fusion), distance, movement; speaks the answer via Piper. Vital signs (breathing/heart) are deliberately never answered — they stay raw bench data. Verified live on the Pi ("what is the humidity" → "The humidity is about 49.2 percent.", spoken aloud). 116 tests pass. **Next layer (the "ears"):** wake-word + Whisper speech-to-text so "Hi Lullaby, what's the humidity?" works fully spoken — needs whisper.cpp + a wake/VAD approach on the Pi and RAM planning (Ollama 1B already loaded; ~475 MB free).
- **2026-06-28** — **"Mic hears everything" + sensor fusion.** (1) Wired the PIR↔radar **scene** fusion (`context.describe_presence_scene`: settled/active near the cot, movement elsewhere, room still...) into the `sensors-live` readout and the narration. (2) Added `detector.classify()` surfacing curated **non-cry YAMNet sounds** (cooing = Coo/Babbling/Gurgling, laughing, talking, snoring, fussing, coughing, sneezing, hiccup) from a single inference — the cry path is unchanged. New `lullaby sounds-live` gives a live readout; when `[sounds].enabled`, the run records observer-only `sound_observed` events (non-cry; crying keeps its own deterministic detector) and the narration summarises them ("Other sounds heard: cooing 2 times..."). Verified live on the Pi (speech → `talking 0.97`). 106 tests pass. **Next (Mo's pick: sounds first, then assistant):** the voice assistant — "Hi Lullaby, what's the humidity?" = wake-word → Whisper speech-to-text → Ollama answering from live sensors → Piper voice (RAM-tight on the Pi, needs planning).
- **2026-06-28** — **Sensor-overlap decision refined (Mo): KEEP the PIR, drop the VL53L0X.** Beyond WiFi-independence, the PIR's longer range (~3–7 m) and wide cone cover whole-room *movement* that the radar's focused forward beam (tuned for close presence + vitals, ~≤1.5 m vitals / a few m presence) misses. Roles are complementary, not redundant: PIR = wide-area movement net ("did anything move anywhere?"); radar = focused cot-zone presence + distance + vitals ("is the baby here, what's their state?"). The VL53L0X (and HC-SR04) are single-point distance with no area niche the radar doesn't already cover → not used for baby monitoring (mechanical/enclosure checks only). No config change: the PIR stays enabled, the VL53L0X stays parked.
- **2026-06-28** — Added BME688 **pressure** (`room_pressure_hpa`, always on, no flag) to the air reader and `sensors-live` readout (shows as hPa). Confirmed the radar exposes 7 entities and **all six readings are active** (presence, illuminance, distance, target count, breathing, heart); the only non-routed entity is the RGB LED (an output, not a reading). Radar 0/negative breathing/heart is now treated as a no-lock sentinel and omitted like NaN, so a far/absent subject never shows a fake `0 bpm` or skews the vitals average (target_count 0 stays valid). Full live suite verified on the Pi — 11 readings across 3 sensors: BME688 temp/humidity/pressure/gas + PIR motion + radar presence/illuminance/distance/count/breathing/heart. 96 tests pass.
- **2026-06-28** — Enabled the BME688 **gas/VOC channel** behind an optional `[sensors.air].gas` flag (off by default) — the experimental nappy-VOC "best guess" path. The air reader configures the gas heater and reports `room_gas_resistance_ohms` once heat-stable; it shows in the `sensors-live` readout as kΩ. Gas resistance is **inverse** to VOC load (high = clean air, low = more VOCs), so a wet/soiled nappy's ammonia makes it drop; the baseline needs a few minutes to settle. Fixed a reader bug where a not-ready measurement cycle (`get_sensor_data()` False, common while the gas heater runs) permanently disabled the air reader, and isolated the gas read so a hiccup can never drop temperature/humidity. Verified live (gas climbed 6→12 kΩ as the heater warmed). Also added `lullaby sensors-live` (clean combined live readout of all enabled sensors). **Sensor-overlap audit (Mo 28 Jun):** the radar replaces the PIR (motion) and the VL53L0X + HC-SR04 (distance) for baby monitoring; the PIR's only remaining value is a WiFi-independent motion backstop. The PIR cannot sense a wet nappy (motion only); the BME688 (temp+humidity+gas) is the nappy path. 96 tests pass.
- **2026-06-28** — **Mo authorised a bench/research-only radar vitals mode** (capture + read out the MR60BHA2's respiratory + heart-rate values). This does NOT change the product rule: vitals are still never a product or medical signal. Implementation keeps five hard guardrails: (1) off by default behind an explicit `[sensors.radar].bench_vitals` flag; (2) the values are surfaced only as raw, clearly-labelled bench data ("raw radar bench data, not a medical or safety signal, and not clinically validated") on every surface; (3) the readout is **deterministic** (`radar_vitals.py`), never LLM-generated, so the small model can't fabricate a vital; (4) vitals are never fed into the product narration (excluded from `_environment_facts`, and `heart`/`respiratory`/`pulse`/`breathing` remain banned narration words) and never paired with reassurance; (5) vitals never drive detection, timing, alerts, or soothe. New `lullaby radar-vitals` command gives a live readout + a labelled period summary; the main run can also speak the labelled summary when `bench_vitals` is on. NaN/no-lock readings are treated as "no reading", never a fabricated number. Captured under namespaced keys `radar_respiratory_rate` / `radar_heart_rate_bpm`. Product/clinical use of vitals still requires the Tier-3 gate + mentor sign-off. 87 tests pass.
- **2026-06-28** — Integrated the Seeed MR60BHA2 60GHz mmWave radar (XIAO ESP32C6, pre-flashed ESPHome) as an optional, off-by-default sensor, read over WiFi via the ESPHome native API (`aioesphomeapi`; no USB cable, no flashing, no encryption — onboarded over its own WiFi captive portal AP `seeedstudio-mr60bha2` at `192.168.4.1`; live on the home WiFi at `192.168.1.146:6053`). A new `Mr60RadarReader` holds a background connection (reconnect-with-backoff) and serves a non-blocking cached snapshot, so it never perturbs detection (radar live in ~2s on the Pi). Only **non-medical** fields feed the narration — presence, room brightness (illuminance), target distance and count as best-guess context; **respiratory and heart rate are deliberately never routed** and stay bench-only (Tier 3, no vital-sign claims). Hardened the narrator against small-model fabrication: explicit no-invent/no-preamble prompt, `heart`/`respiratory`/`pulse` added to the banned-word backstop, and first-paragraph trimming of stray trailing lines. Fixed a reader bug where `asyncio` was unimported in `_stream` (NameError killed the loop before any state arrived). Verified live end-to-end on the Pi: faithful spoken recap including real brightness + presence, no fabricated values, no vital signs. 77 tests pass.
- **2026-06-28** — Wired the verified BME688 air sensor (`0x76`) and HC-SR501 PIR motion sensor (GPIO4) into the run as optional, off-by-default derived context. Sensor reads append `environment_sample` events only, degrade to empty readings when hardware/libraries/tools fail, feed narrator room-temperature/humidity and movement-count wording, and never control cry detection, timing, soothing, or alerts.
- **2026-06-28** — Added an optional local narrator + Piper voice-out layer. It is off by default, runs only after the finished `NightReport`, receives derived text only, degrades to the deterministic digest if Ollama, TTS, or playback is unavailable, and never controls detection, timing, soothe decisions, or alarms.
- **2026-06-28** — Demo-day build progress on the Pi. **AI-orchestrator-first decision (Mo):** a local AI brain is on by default and runs the experience, with a deterministic cry→alert reflex that always fires the alert even if the AI lags; this replaces the old "works-with-AI-off" rule (any cloud LLM stays optional, behind a flag, derived-text-only). Enabled I²C and verified sensors: BME688 air sensor answers at `0x76`, the HC-SR501 PIR is wired on GPIO4 (3.3V output, Pi-safe), and both Camera Module 3 units are detected. Enabled the MAX98357 I²S overlay with `,no-sdmode` (frees GPIO4 for the PIR), but the 3W speaker is not yet soldered to the amp, so demo audio runs through the **Anker Bluetooth** speaker (PipeWire default sink; players forced via `SDL_AUDIODRIVER=pulse`). Added `RealtimeWavFileAudioSource` + `analyze --realtime` and moved mic resampling off the audio callback with `latency=high` (commit `fb0fa31`); 46 tests pass. **Confirmed the full live loop on the Pi:** a real cry through the USB mic scored 0.891 and triggered soothe; a file-fed `analyze --realtime` cry+silence run played soothing music through the Anker and resolved after 2 quiet checks ("crying no longer detected"). Adopted the vented-base + plush architecture; OLED face + one micro-servo are a bench-only demo exception; camera is daylight-motion only; radar (MR60BHA2) and night vision stay post-demo.
- **2026-06-27** — Corrected the recorded VL53L0X occupancy from pin 19 to pin 17 after Mo clarified the wire is on physical pin 17. Current VL53L0X pins are 17/20/16/15: 3.3V, GND, SCL/GPIO23, SDA/GPIO22.
- **2026-06-27** — Initially recorded Mo's Raspberry Pi 5 bench GPIO occupancy as BME688 on pins 1/3/5/9, MAX98357 on 35/12/40/4/6, HC-SR501-style PIR on 7/14/2, and VL53L0X on 19/20/16/15; superseded by the pin-17 correction above.
