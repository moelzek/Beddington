#!/usr/bin/env bash
# Gated batch-loop for the "soothe >= 10 min after a cry" change (M1).
# codex writes the unit, the driver runs the FULL test suite as a gate, then
# commits + pushes to main. Stops on a blocked unit. Resumable.
...
LEDGER="$STATE_DIR/soothe-units.md"; SUMMARY="$STATE_DIR/SOOTHE_SUMMARY.md"
SPEC="plans/soothe-duration-spec.md"
mkdir -p "$LOG_DIR"
...
UNITS=(
  "M1|feat: soothe holds for at least 10 minutes after a cry|Add soothe.min_play_seconds (default 600); SootheController must not stop (settle/quiet) before the minimum elapses, and the sound keeps playing that long. Config + validation + tests."
)
...
[ -f "$LEDGER" ] || { echo "# soothe-loop ledger"; echo; } > "$LEDGER"
...
for entry in "${UNITS[@]}"; do
  ...
  while [ "$attempt" -le "$MAX_RETRIES" ]; do
    ...
    PROMPT="Read $SPEC in this repo IN FULL — it explains the soothe early-stop bug and the required minimum-hold behaviour with acceptance tests. Implement ONLY unit $UNIT ($TASK). Match existing code/test style. Ensure the WHOLE suite passes with: $TEST_CMD . stdlib only — no new dependency. Do not change deterministic cry detection/timing/notifications. When done print one line of what changed.$extra"
    ...
    git commit -m "$MSG" -m "Built by gated codex soothe-loop ($UNIT)." -m "Co-Authored-By: WOZCODE <contact@withwoz.com>" >> "$LOG_DIR/git.log" 2>&1
...
{ echo "# soothe-loop summary — $(date '+%Y-%m-%d %H:%M')"; echo; echo "## Ledger"; cat "$LEDGER";
  echo; echo "## Recent commits"; git log --oneline -6; } > "$SUMMARY"
log "soothe-loop finished — summary at $SUMMARY"; cat "$SUMMARY"
