...
dry / dim / clean) using ordinary nursery comfort ranges, framed as gentle
guidance, never a medical or safety claim. Vital signs (breathing/heart) are
deliberately never answered.

Routing is intent-based: specific readings are matched first; a general
"how is the room?" gives a whole-room overview; "how many people?" counts; and
"is anyone there?" answers presence plainly.
"""
...
# fallback) before any branch can answer it — the medical-claim boundary. These
# are intentionally checked first so "how many breaths" / "is she breathing"
# never reach the people-count, presence, or motion branches.
_VITALS_WORDS = (
...
def _mentions(question: str, *keywords: str, fuzzy: tuple[str, ...] = ()) -> bool:
    ...
    'shot' != 'hot'; substring for phrases), or a word within edit-distance 2 of
    a fuzzy target (so a misheard 'temprature' still matches 'temperature')."""
...
def _bright_label(lux: float) -> str:
    if lux < 10:
        return "it's dark"
    if lux < 50:
        return "it's dimly lit"
    return "it's softly lit" if lux < 200 else "it's bright"
...
def _pressure_phrase(p: float) -> str:
    return f"The air pressure is about {p:g} hectopascals — {_pressure_label(p)}."


def _air_quality_phrase(ohms: float) -> str:
    return (
        f"The air quality {_air_label(ohms)} — about {ohms / 1000:.0f} kilo-ohms "
        "(higher is cleaner)."
    )


def _brightness_phrase(lux: float) -> str:
    extra = ", good for sleep" if lux < 10 else ""
    return f"The room is about {lux:g} lux — {_bright_label(lux)}{extra}."
...
def _people_phrase(snapshot: dict[str, object]) -> str:
    count = snapshot.get("target_count")
    if not isinstance(count, bool) and isinstance(count, (int, float)):
        n = int(count)
        if n <= 0:
...
def _room_overview(snapshot: dict[str, object]) -> str:
    ...
    if t is not None:
        parts.append(f"it's about {t:g} degrees, {_temp_label(t)}")
    h = _num(snapshot, "room_humidity_pct")
    if h is not None:
        parts.append(f"humidity is {h:g} percent, {_humid_label(h)}")
...
def answer_question(question: str, snapshot: dict[str, object]) -> str:
    ...
    # question. The brain has no vitals reading and must never imply one.
    ...
    if _mentions(q, "humid", "muggy", "dry", "damp", "humidity", fuzzy=("humidity",)):
    ...
    if _mentions(q, "light", "bright", "dark", "dim", "lux", fuzzy=("brightness",)):
    ...
    if _mentions(
        q, "anyone", "anybody", "someone", "somebody", "is there", "present",
        "nearby", "people", "person",
    ):
        return _presence_phrase(snapshot)
